...
public:
	...
	{
		std::cout << "Sales_data(const std::string &s, int n, double dc, double p)" << std::endl;
	}
	Sales_data(std::string s = " ") :Sales_data(s,0,0,0)
	{
		std::cout << "使用了Sales_data(std::string s = " ") :SN(s)" << std::endl;
	}
	Sales_data(const std::string &s,int n) :Sales_data(s,n,0,0)
	{
		std::cout << "使用了Sales_data(const std::string &s) :SN(s)" << std::endl;
	}
